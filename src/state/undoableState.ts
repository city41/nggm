  ...
  switch (action.type) {
    ...
    case "MoveSprite": {
      ...
      const layer = state.layers.find(l => {
        return l.groups.some(g => {
          return g.sprites.some(s => {
            return (
              s.spriteMemoryIndex === spriteMemoryIndex && s.pauseId === pauseId
            );
          });
        });
      });
      ...
      const currentSpriteGroup = layer.groups.find(sg => {
        return (
          sg.pauseId === pauseId &&
          sg.sprites.some(s => s.spriteMemoryIndex === spriteMemoryIndex)
        );
      });
      ...
      const diffX = newComposedX - currentSpriteGroup.sprites[0].composedX;
