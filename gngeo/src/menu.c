...
int menu_loop_result;

void menu_loop(void) {

	main_menu->draw(main_menu);

	menu_loop_result = main_menu->event_handling(main_menu);
}
...
#ifdef __EMSCRIPTEN__
    emscripten_set_main_loop(menu_loop, 60, 1);
#else
while (1) {
    menu_loop();
    if (menu_loop_result > 0) return menu_loop_result;
}
#endif
