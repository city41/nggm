...
(function() {
  var loadPackage = function(metadata) {
    ...
    function runWithFS() {
      ...
      Module["FS_createPath"]("/", "virtualfs", true, true);
      ...
      function processPackageData(arrayBuffer) {
        ...
        Module["removeRunDependency"]("datafile_gngeo.data");
      }
      Module["addRunDependency"]("datafile_gngeo.data");
  ...
  loadPackage({
    files: [
      { start: 0, audio: 0, end: 336, filename: "/virtualfs/gngeorc" },
      {
        start: 336,
        audio: 0,
        end: 221114,
        filename: "/virtualfs/gngeo_data.zip"
      }
    ],
    remote_package_size: 221114,
    package_uuid: "9f441a42-fdda-48bc-9982-6af19d78eb93"
  });
})();
...
function _malloc_or_die() {
  err("missing function: malloc_or_die");
  abort(-1);
}
...
function _memory_region() {
  err("missing function: memory_region");
  abort(-1);
}

function _memory_region_length() {
  err("missing function: memory_region_length");
  abort(-1);
}
...
function _neogeo_bootleg_cx_decrypt() {
  err("missing function: neogeo_bootleg_cx_decrypt");
  abort(-1);
}

function _neogeo_bootleg_sx_decrypt() {
  err("missing function: neogeo_bootleg_sx_decrypt");
  abort(-1);
}
...
var _get_current_pal_addr = (Module["_get_current_pal_addr"] = function() {
  assert(
    runtimeInitialized,
    "you need to wait for the runtime to be ready (e.g. wait for main() to be called)"
  );
...
var _get_neogeo_frame_counter = (Module[
  "_get_neogeo_frame_counter"
] = function() {
  assert(
    runtimeInitialized,
    "you need to wait for the runtime to be ready (e.g. wait for main() to be called)"
...
var _get_neogeo_frame_counter_speed = (Module[
  ...
  return Module["asm"]["_get_neogeo_frame_counter_speed"].apply(
    null,
    arguments
  );
});
...
var _run_rom = (Module["_run_rom"] = function() {
  assert(
    runtimeInitialized,
    "you need to wait for the runtime to be ready (e.g. wait for main() to be called)"
  );
  ...
  return Module["asm"]["_run_rom"].apply(null, arguments);
});
